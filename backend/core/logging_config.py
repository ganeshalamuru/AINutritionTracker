...
# logs. request_id/user_id are injected onto every record by the log-record factory
# (core.request_context), so these fields are always present (default "-").
_FORMATTER = logging.Formatter(
    "%(asctime)s.%(msecs)03d [%(levelname)s] [%(threadName)s] "
    "[req:%(request_id)s u:%(user_id)s] %(name)s: %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
...
def configure_logging(level=None):
    ...
    # request-context middleware has reset the contextvars, so it can't carry req/user
